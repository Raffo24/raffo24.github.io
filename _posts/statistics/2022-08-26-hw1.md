...
5. **Interval Count Determination**: The size of intervals (`intervalSize`) depends on thresholds (e.g., if breaches are greater than 50, 100, 200, etc.), different size of intervals are used to provide meaningful visualization:
```javascript
    const serversCount = servers.length;
    intervalSize = 1
    if (serversCount >= 10) {
...
6. **Interval Size Calculation**: The interval number is calculated by dividing `serversCount` by `intervalSize`, which determines the range of breaches each interval will cover. An array `intervals` is created, with each entry representing the lower and upper bounds of each interval:
   ```javascript
    const intervalCount = Math.ceil(serversCount / intervalSize);
    const intervals = Array.from({ length: intervalCount }, (_, i) => `${i * intervalSize}-${(i + 1) * intervalSize}`);
   ```
